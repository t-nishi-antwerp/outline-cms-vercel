...
- ブラウザキャッシュをクリア
- プライベートブラウジングモード（シークレットモード）で確認

**重要:** 最新版では、APIのサーバー側キャッシュは完全に無効化されています（`Cache-Control: no-cache, must-revalidate`）。公開後、APIは即座に最新データを返します。ブラウザキャッシュのみが影響する可能性があります。
